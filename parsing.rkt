...
'(error "<hey><html>")
|#
(define (parse-html-tag str) (if (string=? "<html>" (substring str 0 6)) 
                                 (list "<html>" (substring str 6)) (list 'error str)))
...
'(error "goodbye hi")
|#
(define (make-text-parser t)  (lambda(k)(if (string=? t (substring k 0 (string-length t))) 
                                 (list t (substring k (string-length t))) (list 'error k))))
